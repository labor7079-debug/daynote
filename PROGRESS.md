...
## 현재 위치: **v0.2.0 업데이트 업로드 대기(2026-07-02)** — 첫 배포(내부 테스트) 완료 후 UI 개선 반영, AAB·MSI 재빌드됨
...
### 🔼 v0.2.0 업데이트 (2026-07-02 오후 — 업로드 대기)
- **포함**: 기간 할 일 seamless bar(`e8edf16`) + UI 5종(`62b719d`): 에디터 나가기 확인 다이얼로그·Ctrl+S 저장·글씨체 통일(세리프 제거)·설정 톱니 하단바 이동·우측 하단 동기화 FAB.
- **버전**: versionCode **2** / versionName **0.2.0** / 데스크톱 packageVersion **1.1.0**(MSI 제자리 업그레이드용 — 앞으로 업데이트마다 함께 범프).
- **산출물(15:42 생성)**: AAB `app/build/outputs/bundle/release/app-release.aab`(13.3MB, 서명) · **MSI `app/build/compose/binaries/main/msi/DayNote-1.1.0.msi`(68MB)** · 폴더형 `.../app/DayNote/`.
- **개발자 할 일**: ① Play Console → 내부 테스트 → 새 버전 만들기 → AAB 업로드(`2 (0.2.0)` 자동 인식) → 출시 노트 → 저장 및 출시. ② PC는 MSI 실행(기존 1.0.0 위에 업그레이드, SmartScreen "추가 정보→실행"). ③ 기기 검증 체크리스트(아래 ①) + seamless bar·UI 5종 확인.
...
## 🚦 다음 세션 시작 가이드 (2026-07-02 마감 기준)

**상태**: 작업트리 clean, `42f1fd0` 푸시됨. **assembleDebug + desktopTest 33건 통과.** DB 스키마 **v5**(v4→v5: `tasks.endDate`).

**① 기기 검증 대기(개발자 — 새 APK 설치 후 한 번에)**:
- [ ] 마이그레이션: 업데이트 설치 후 기존 메모·할 일 유지 확인 (v4→v5)
- [ ] 캘린더: 날짜 탭→상세 표시 · 상세 좌우 스와이프(전날/다음날) · 주 헤더 "(W27)" · 공휴일 음영+빨간 표시(10월로 확인) · 할 일 내용 박스(접힘/펼침 동일)
- [ ] 기간 할 일: 상세 추가란 "기간: 종료일 지정" → 3일짜리 생성 → 월 달력 bar 걸침 · 이동/복사 시 기간 유지
- [ ] 홈 위젯 2종(DayNote / DayNote 월 달력): 배치·데이터 변경 시 갱신·다크모드
- [ ] 항목 탭=미니 캘린더 팝업 · 길게 누름=이동/복사/삭제 메뉴 (메모탭·검색·To-Do·캘린더 상세)
- [ ] 구글 캘린더 무음 재로그인: 재시작 후 로그인 없이 동기화 (최초 1회 동의는 필요)
- [ ] 당겨서 새로고침 동기화 · 저장/추가 스낵바 · 비슷한 메모 추천 · AI 이력("이전 결과 보기") · 종일 할 일 오전 9시 알림 · 메모/To-Do 날짜 그룹+기간 필터
- [ ] v0.2.0 신규: 에디터 뒤로(내용 수정 후) → "저장하지 않고 나가시겠습니까?" · Ctrl+S 저장(키보드 연결 시) · 하단바 톱니로 설정 진입 · 우측 하단 동기화 FAB · 글씨체 통일 · 월 달력 기간 bar 틈 없이 이어짐

**② 다음 작업 후보(우선순위 제안)**:
1. **Supabase `end_date` 컬럼 추가**(개발자: 대시보드에서 `ALTER TABLE tasks ADD COLUMN end_date bigint;`) → 클라이언트 `TaskRow`/`RowMappers` 반영(한 파일) — 기간 할 일 멀티기기 동기화
2. **반복 일정**(DB v5→v6 + Supabase 컬럼 + UI) — 세션급
3. **구글 캘린더 pull**(3-B3c, remoteId 있는 항목 양방향) — 세션급, 기기 검증 필수
4. 잔여 폴리시: 5-B 폴드 힌지 정밀 · 5-C3 ML Kit 필기 변환 · 데스크톱 .exe 아이콘 · 월 달력 bar 칸 사이 여백 조정(사용자 피드백 대기)

**③ 참고**: 공휴일 테이블(`domain/holiday/KoreanHolidays.kt`)은 2025~2027 내장 — 매년 6월 말 우주항공청 월력요항 발표 시 한 해치 추가. 임시공휴일 지정 시 한 줄 추가.
