...
- **버전**: versionCode **9** / versionName **0.5.0** / MSI **1.4.0**.
- **산출물(17:09)**: AAB `app/build/outputs/bundle/release/app-release.aab`(30.6MB, 서명) · MSI `app/build/compose/binaries/main/msi/DayNote-1.4.0.msi`(85.4MB) · 폴더형 `.../app/DayNote/`.
- **개발자 할 일**: ① Play Console → 내부 테스트 → `9 (0.5.0)` AAB 업로드(4~8 건너뜀). ② PC 는 MSI 1.4.0 설치 → 설정 → 구글 캘린더 동기화 → 동기화 사용 ON → **"구글 로그인"(브라우저 동의 1회)** → "캘린더 목록 불러오기" → 공유 캘린더 체크 → 달력에 표시 확인. ③ 재시작 후 로그인 없이 자동 동기화되는지 확인(무음 갱신).
...
- [ ] v0.2.1 신규: 기간 bar 시작·종료 끝이 날짜 타일 안에서 마감 · "+N개 더" 탭 → 아래로 펼침/"접기"(월 달력 칸·주 아젠다) · 위젯 기본 크기(하루 2x2·월 4x2) + 기존 위젯 크기 조절 가능 여부
...
4. 잔여 폴리시: 5-B 폴드 힌지 정밀 · 5-C3 ML Kit 필기 변환 · 데스크톱 .exe 아이콘 (bar 칸 여백은 v0.2.1 에서 반영 완료)
