...
- **홈 위젯 시각 병기**(사용자 요청): 두 위젯(`DayNoteWidgetProvider` 하루 목록 · `DayNoteMonthWidgetProvider` 우측 오늘 목록)의 할 일 줄에 시각을 앞에 표기 — 시각만 지정 "14:00", 범위 "14:00~16:00", 여러 날 "7/2 14:00~7/4 16:00", 종일은 표기 없음. `TaskEntity.toDomain().scheduleLabel()` 사용.
- **데스크톱(PC)**: 캘린더 화면이 `commonMain` 공유 코드라 별도 수정 없이 같은 규칙 적용(v0.5.2 부터 이미 시각 표기, 이번엔 위젯과 규칙 일원화). MSI 재빌드로 반영.
- **버전**: versionCode **12** / versionName **0.5.3** / MSI **1.4.4**.
- **산출물**: (빌드 대기) AAB `app/build/outputs/bundle/release/app-release.aab` · MSI `app/build/compose/binaries/main/msi/DayNote-1.4.4.msi`.
- **개발자 할 일**: ① 산출물 빌드(AAB/MSI) → Play Console 내부 테스트 `12 (0.5.3)` 업로드 · PC MSI 1.4.4 제자리 업그레이드. ② 기기 검증: 홈 위젯(작은/월)에서 시각 지정 할 일이 "14:00 …"처럼 시각과 함께 표기 · 종일 할 일은 시각 없음 · 앱/데스크톱 캘린더와 표기 일치.
