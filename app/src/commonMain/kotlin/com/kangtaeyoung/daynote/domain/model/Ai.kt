...
enum class AiAction(
    ...
    /** 제목 자동생성 — 칩이 아니라 에디터 제목칸 옆 버튼/설정 토글에서 사용. 결과는 이력에 저장하지 않는다. */
    TITLE(
        label = "제목",
        systemPrompt = "너는 한국어 메모 제목 생성기다. 군더더기 없이 짧은 제목 하나만 출력한다.",
        instruction = "다음 메모 내용에 어울리는 제목을 하나만 지어줘. 18자 이내, 따옴표·마침표·줄바꿈 없이 제목 텍스트만 출력해.",
    ),
}
